...
演示目标不是展示内部 agent、MCP 或 GraphRAG 机制，而是展示：

```text
资料库证据 -> 多个问题并行求证 -> 带引用答案 -> 组织章节 -> 生成可复核草稿
```

建议演示顺序：

1. 打开一个已经准备好的 Writing board。
2. 展示 board 已绑定当前知识库范围，避免答案跨资料库。
3. 运行 2 到 3 个 question 节点，等待 answer 节点出现。
4. 展开一个 answer 节点，只展示健康状态、引用和“查看原文”，不要解释 FastReAct/MCP 等工程细节。
5. 把 answer 加入右侧章节。
6. 点击“生成章节草稿”。
7. 展示草稿中的结论、证据引用、缺口/风险提示。

演示成功标准：

- 用户看得懂“为什么这段话可信”：每个关键答案有引用或 source ref。
- 用户看得懂“资料范围是什么”：board/节点显示 KB scope。
- 用户能继续编辑：draft 是 Markdown/正文，不是不可修改的最终报告。
- UI 不暴露内部机制词：草稿和节点正文不应出现 `FastReAct`、`MCP`、`tool_call`、`GraphRAG`。

当前成熟度判断：

- 适合内部和小范围客户试演。
- 适合销售负责人理解产品方向。
- 不宜承诺为完整自动写报告系统；当前定位是 evidence-grounded writing workspace。
