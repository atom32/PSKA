...
- `frontend/e2e/multi-kb-scoped-ask.spec.ts` 已扩展真实浏览器验收：UI Ask 展示 alpha citation inspector，点击“查看原文”后 ReaderPane 包含 alpha 原文且不包含 beta secret；点击“追问这段”后输入框包含 alpha `source_item_id`；点击“生成 Brief”后会进入 Writing，最新 brief 节点包含 alpha 证据且不包含 beta secret；Graph Ask 在空图/有图主路径也会显示 `graph-path-evidence-health` 且不泄露 beta 证据，点击“保存到 Writing”后进入 Graph Brief board 且 answer/evidence 节点保留 alpha citation；真实 linking digest 生成的 Review candidate 会在 Today 右栏显示 `today-review-evidence-health`，并在 Review Center 显示 `review-evidence-health`，点击“批准并应用”后会进入已应用列表并创建 graph relationship。
...
- `npm run e2e:multi-kb-ask` 通过，使用 AuthNode/Gateway browser session 覆盖多 KB hard scope、ReaderPane 高亮、source-focused follow-up、Ask -> Writing Brief、Graph Ask 健康摘要、Graph Ask -> Writing evidence 和 Review approve/apply；临时 alpha/beta/review/graph/writing 数据清理后无残留。
...
- 继续收紧 Ask/Reader/Writing 的细节验收：补充 ProcessingTimeline、Writing/Review/Graph 健康摘要的真实运行截图核对，并把 Graph 节点选中 -> Writing evidence、Review applied relationship -> Graph inspection 等后续产品动作做成更完整的浏览器验收。
