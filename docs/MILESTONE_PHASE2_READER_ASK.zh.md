...
- 同一 spec 已补充空 KB 联动空态验收：Digest tab 的 `knowledge-base-digest-empty` 会显示当前 KB cockpit、禁用空库 Digest 动作并可跳到资料/处理；Graph tab 的 `knowledge-base-graph-empty` 可跳到处理或 Graph 工作区，Writing tab 的 `knowledge-base-writing-empty` 可跳到 Ask、新建当前 KB 画布或打开 Writing 工作区。
